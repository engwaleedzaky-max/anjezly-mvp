# file: app.py
"""
Service Marketplace MVP (Step 2) - Simple Login (Session)
- Simple login with role (customer/provider) + name (no password)
- Session cookie via Starlette SessionMiddleware
- Provider-only access to provider dashboard and status updates
...
{% if not user or user.role != 'provider' %}
  <div class="card">
    <h3 style="margin-top:0;">لوحة مقدم الخدمة</h3>
    <p class="muted">يجب تسجيل الدخول بدور <b>provider</b> لعرض لوحة مقدم الخدمة.</p>
    <a class="badge" href="/login">اذهب لتسجيل الدخول</a>
  </div>
{% else %}
  <div class="card">
    <h3 style="margin-top:0;">لوحة مقدم الخدمة</h3>
    <p class="muted">أنت مقدم الخدمة: <b>{{ user.name }}</b>. اقبل الطلبات الجديدة ثم افتح التفاصيل لتغيير الحالة.</p>
...
<div class="card">
  ...
  {% if not user or user.role != 'provider' %}
    <p class="muted">يجب تسجيل الدخول كمقدم الخدمة لتحديث الحالة.</p>
  ...
  <div class="row" style="justify-content: space-between; align-items:center; margin-top: 12px;">
    <a class="badge" href="/provider">لوحة مقدم الخدمة</a>
...
def get_session_user(request: Request) -> Optional[SessionUser]:
    ...
    role = raw.get("role")
    name = raw.get("name")
    if role not in ("customer", "provider") or not isinstance(name, str) or not name.strip():
        return None
...
def update_status(
    ...
    *,
    ...
    with closing(get_conn()) as conn, conn:
        ...
        # صلاحيات: القبول يتطلب اسم مقدم الخدمة، ولا يمكن قبول طلب مقبول مسبقًا
        ...
        # صلاحيات: أي تحديث بعد القبول يجب أن يكون من نفس مقدم الخدمة الذي قبل الطلب
        if current in ("accepted", "in_progress"):
            if not provider_name:
                raise HTTPException(status_code=403, detail="Provider required")
...
def accept_request(request: Request, request_id: int):
    ...
    update_status(request_id, new_status="accepted", provider_name=user["name"])
...
def set_status(
    ...
    user = require_role(request, "provider")
    update_status(request_id, new_status=new_status, provider_name=user["name"])
    return RedirectResponse(f"/requests/{request_id}", status_code=303)