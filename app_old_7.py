# file: app.py
"""
أنجزلي - Service Marketplace MVP (Passwords + Admin PIN + Reset Codes)
Brand:
- Name: أنجزلي
- Slogan: اطلبها، وإحنا ننجزها

Auth:
- طالب الخدمة (customer): register/login with password
- مقدم الخدمة (provider): register/login with password
- admin: login with PIN only

Admin:
- Add services dynamically
- Assign/reassign provider from dropdown list (Force option)
- View and manage password reset codes (support)

DB:
- Uses SQLite (app.db)
- Migration supports old service_type and new service_name and old NOT NULL constraints
"""
...
def init_db() -> None:
    with closing(get_conn()) as conn, conn:
        ...
        # Migration: service_type -> service_name
...
<head>
  ...
  <style>
    ...
    .btn-primary { border: none; background: var(--accent); color: #fff; font-weight: 800; }
    .btn-primary:hover { opacity: .92; }
    .btn-secondary { background: #fff; border: 1px solid var(--border); }
    .btn-secondary:hover { border-color: #cbd5e1; }
    .btn-danger { border: none; background: #b00020; color: #fff; font-weight: 800; }
